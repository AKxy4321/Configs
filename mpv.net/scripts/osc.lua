...
local user_opts = {
    ...
    fadeduration = 300,         -- duration of fade out (and fade in, if enabled) in ms, 0 = no fade
    fadein = true,             -- whether to enable fade-in effect
    deadzonesize = 0.35,           -- size of deadzone
    ...
    scrollcontrols = false,      -- allow scrolling when hovering certain OSC elements
    title = "${playlist-count>1:[${playlist-pos-1}/${playlist-count}] }${filename}",
    ...
    windowcontrols_title = "${filename}", -- same as title but for windowcontrols
...
local function bar_layout(direction, slim)
	...
	local function P(v)  return math.floor(v * PAD_SCALE + 0.5) end
    ...
    local osc_geo = {
        ...
        w = nil,
        h = slim and S(25) or S(56),
    }

    local padX = P(9)
    local padY = P(3)
    local buttonW = S(27)
    local tcW = (state.tc_ms) and S(170) or S(110)
    ...
    local line1 = osc_geo.y - direction * (S(9) + padY)
    local line2 = osc_geo.y - direction * (S(36) + padY)
...
local function osc_init()
    ...
    ne.content = function ()
        local title = state.forced_title or
                      mp.command_native({"expand-text", user_opts.windowcontrols_title})
        title = title:gsub("\n", " ")
        return title
    end
